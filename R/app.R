...
# IMPORTANT: this part is commented because it is
# time-consuming (see README.md and run in a server if you
# want to get `res_p3` by yourself) Period 3: june 2020 to
# april 2022 start <- Sys.time() res_p3 <-
# mlsclust(sc2_tre_curated, sc2_md_curated,
# min_descendants=10, max_descendants=20e3,
# min_cluster_age_yrs=1/12,
# \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tmin_date=as.Date('2019-12-30'),
...
# \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\troot_on_tip='Wuhan/WH04/2020',
# root_on_tip_sample_time=2019.995, detailed_output=FALSE,
# ncpu=15) end <- Sys.time() total_time_p3 <- as.numeric
# (end - start, units = 'mins') message(paste('Total time
# elapsed:',total_time_p3,'mins')) # ~44 hours
# saveRDS(res_p3, 'rds/res_p3.rds') res_p3 <-
# readRDS('rds/res_p3.rds') #readRDS(system.file('extdata',
# 'res_p3.rds', package='mlscluster'))
res_p3 <- readRDS(url(
  "https://osf.io/9tnck/download",
  "rb"
))
...
for (i in 1:length(thr)) {
  run_diff_thresholds(
    stats_df_unfilt = res_p3[[1]],
    tgt_nodes = res_p3[[2]],
    homoplasies = res_p3[[3]],
    output_dir = glue(
      "results/03_sc2_whole_period/threshold_quantile_{thr[[i]]}/"
    ),
    quantile_choice = quantl[i],
    quantile_threshold_ratio_sizes = thr[i],
    quantile_threshold_ratio_persist_time = thr[i],
    ...
    plot_global_mut_freq = FALSE,
    detailed_output = FALSE
  ) # desc_sisters=list(res_p3[[4]], res_p3[[5]]), amd=res_p3[[6]]
}
end <- Sys.time()
...
# 'Delta_other','Omicron_BA.1.*','Omicron_BA.2.*','Other')
# #'Beta_B.1.351','Gamma_P.1','Omicron_BA.*',
major_lineages <- c(
  "B.1.177.*",
  "B.1.1.7",
  "AY.4.*",
...
rmult_p3 <- stats_multiple_thresholds(
  "results/03_sc2_whole_period",
  pal_lineages,
  "period3",
  "ED_FileS2.txt"
)
...
ggsave(
  "stat_results/plots_paper/ED_FigS2.png",
  plot = ed_f2,
  units = "px",
  width = 2200,
  height = 2000,
...
ggsave(
  "stat_results/plots_paper/ED_FigS4.png",
  plot = s4,
  units = "px",
  width = 1500,
  height = 2000,
...
# Extended Data Figs S3-S8 (top30 regardless of threshold
# for main major lineages: AY.4, B.1.1.7, AY.*, BA.1, BA.2)
...
for (i in major_lineages_supp_indices) {
  ...
  ggsave(
    glue(
      "stat_results/plots_paper/ED_FigS{j}_{major_lineages[i]}.png"
    ),
    plot = rmult_p3[[1]][[i]] +
...
# rmult_p3[[1]][[1]] #B.1.177.* rmult_p3[[1]][[7]] #Other
# lineages

sngrt_r3 <- stacked_nsites_genomic_region_mult_thresholds(
  "stat_results/period3/genomewide_plot_non-syn/"
)

## End run for multiple thresholds ##

## 3. Run FDR ## Extracting mutations from the entire
## dataset of >1.2 million sequences from England
## cog_md_muts_p2 <-
## extract_muts_period('rds/sc2_md_curated_WITH_Xs_Ns.rds',
## as.Date('2021-11-15')) # this might need >16 GB of RAM
## cog_md_muts_p3 <-
## extract_muts_period('rds/sc2_md_curated_WITH_Xs_Ns.rds',
## as.Date('2022-04-30'))
cog_md_muts_p2 <- extract_muts_period(
  url("https://osf.io/f953r/download", "rb"),
  as.Date("2021-11-15")
) # this might need >16 GB of RAM
cog_md_muts_p3 <- extract_muts_period(
  url("https://osf.io/f953r/download", "rb"),
  as.Date("2022-04-30")
)

# Computing codon positions of mutations
muts_match_codons_p2 <- compute_muts_match_codons(
  cog_md_muts_p2[[1]],
  cog_md_muts_p2[[2]]
)
muts_match_codons_p3 <- compute_muts_match_codons(
  cog_md_muts_p3[[1]],
  cog_md_muts_p3[[2]]
)

fdr_plots_p2 <- fdr_mlsclust(
  "results/02_sc2_root_to_nov2021",
  muts_match_codons_p2[[1]],
  muts_match_codons_p2[[2]],
  muts_match_codons_p2[[3]],
  "period2"
)
fdr_plots_p3 <- fdr_mlsclust(
  "results/03_sc2_whole_period",
  muts_match_codons_p3[[1]],
  muts_match_codons_p3[[2]],
  muts_match_codons_p3[[3]],
  "period3"
)
...
rem_labels <- function(ggplot_obj_list) {
  for (i in 1:length(ggplot_obj_list)) {
    if (i <= 20) {
      ggplot_obj_list[[i]] <- ggplot_obj_list[[i]] +
        theme(
          axis.text.x = element_blank(),
          axis.title.x = element_blank()
        )
...
ggsave(
  glue(
    "stat_results/period2/fdr_codons/fdr_all_regions_p2.png"
  ),
  width = 10,
  height = 10,
  dpi = 600,
...
# Extended data Fig S3. FDR and epsilon error rates overall
# and for each genomic region
fdr_plots_p3 <- rem_labels(fdr_plots_p3)
fdr_plots_p3_all <- ggarrange(
  plotlist = fdr_plots_p3,
  ncol = 5,
  nrow = 5,
  common.legend = T
)
annotate_figure(
  ...
  bottom = text_grob("Thresholds", size = 8)
)
ggsave(
  glue("stat_results/plots_paper/ED_FigS3.png"),
  units = "px",
  width = 2200,
  height = 2500,
  dpi = 300,
...
r3 <- stats_selected_threshold(
  "results/03_sc2_whole_period",
  thr_index = 5,
  pal_lineages,
  out_folder
)
# Table 1: abs_freq/nonsyn_top30_muts.csv; Table S1:
# abs_freq/nonsyn_top100_muts.csv

# Figure 3A: number of identified sites for each genomic
# position stacked by threshold
f5_configs <- theme(
...
sngrt_r3 + f5_configs
# Figure 3B: stacked bar counts of unique homoplasies total
# & norm
leg <- theme(
...
f3 <- ggarrange(
  r2[[2]][[1]] + rm_titles_all + rm_axis_text_x + marg,
  r2[[2]][[2]] + rm_titles_all + rm_axis_text_x + marg,
  r3[[2]][[1]] + rm_titles_y + marg,
  r3[[2]][[2]] + rm_titles_y + marg,
  align = "h",
  vjust = 0.5,
  nrow = 2,
  ncol = 2,
...
ggsave(
  "stat_results/plots_paper/Fig3.pdf",
  units = "px",
  width = 2100,
  height = 1850,
  dpi = 300,
...
# Figure 4: A. comparison against positive selection
# (HyPhy) and B.  identified sites by major lineage
r3[[4]] #+ theme(axis.text=element_text(size=5))
ggsave(
  "stat_results/plots_paper/Fig4.pdf",
  units = "px",
  width = 2100,
  height = 1850,
  dpi = 300,
...
ggsave(
  "stat_results/plots_paper/Fig5.pdf",
  units = "px",
  width = 2000,
  height = 1750,
  dpi = 300,
...
ggsave(
  "stat_results/plots_paper/ED_FigS11.png",
  units = "px",
  width = 2000,
  height = 1400,
  dpi = 300,
...
# Extended Data Fig. S1: results for spike when performing
# 99% quantile frequency outlier removal instead of later
# adopted alignment-aware method (NOTE: not reproducible
# here because relying on previous version of the package)
# change_lineage_labels('results/99_quant_rm/03_sc2_whole_period')
# r3_99_rm <-
# stats_selected_threshold('results/99_quant_rm/03_sc2_whole_period',
# thr_index=5, pal_lineages, '99_quant_rm/period3_thr2')
# s1_ab <- ggarrange(r3_99_rm[[2]][[1]],
# r3_99_rm[[2]][[2]], align='h', vjust=0.5, nrow=1, ncol=2,
# labels=c('A','B'), legend='right',
# font.label=list(size=10), common.legend = T) s1_c <-
# r3_99_rm[[3]][[10]] + ylim(2,10) ggarrange(s1_ab, s1_c,
# nrow=2, ncol=1, labels=c('A','B'), font.label =
# list(size=10), common.legend = T)
# ggsave('stat_results/plots_paper/ED_FigS1.png',
# units='px', width=2000, height=1400, dpi=300, bg='white')
2
2
